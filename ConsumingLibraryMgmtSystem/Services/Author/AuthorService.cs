...
namespace ConsumingLibraryMgmtSystem.Services.Author
{
    public class AuthorService : IAuthorService
    {
        ...
        {
            var response = await _httpClient.GetAsync("https://localhost:7238/api/Authors");
        ...
        {
            var response = await _httpClient.GetAsync($"https://localhost:7238/api/Authors/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<bool> AddAuthorAsync(Auth author)
        {
            ...
            var response = await _httpClient.PostAsync("https://localhost:7238/api/Authors", content);
        ...
        {
            var content = new StringContent(JsonSerializer.Serialize(author), Encoding.UTF8, "application/json");
            var response = await _httpClient.PutAsync($"https://localhost:7238/api/Authors/{author.AuthorID}", content);
            return response.IsSuccessStatusCode;
        }
